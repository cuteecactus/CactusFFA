...
import dev.cuteecactus.model.CombatTag;
        ...
        this.taskId = Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, () -> {
            ...
            for (Player player : Bukkit.getOnlinePlayers()) {
                ...
                if (tag.isPresent()) {
                    ...
                    player.sendActionBar(dev.cuteecactus.util.ColorUtil.component(message));
                } else {
                    String idle = plugin.getConfig().getString("ffa.actionbar.idle", "");
                    if (!idle.isBlank()) {
                        player.sendActionBar(dev.cuteecactus.util.ColorUtil.component(idle));
                    }
                }
            }
        }, 20L, 20L);
